"""Run the giveaway synchronization once for diagnostics or operations."""
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(timeout=20, headers={"User-Agent": "SteamRadar/0.1"}) as client:
        ...
        sync = SyncCoordinator(redis, session_factory, monitor, GamerPowerProvider(client))
        count = await sync.sync_giveaways()
        print(f"Synchronized giveaways: {count}")
